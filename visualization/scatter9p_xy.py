...
color=["red","yellow","blue","purple","orange","green","cyan","pink","brown"]
color=['red']*9
s=0.1
alp=0.2
for i, category in enumerate(unique_categories1):
    ...
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[0], marker='o', s=s, label=label,alpha=alp)

for i, category in enumerate(unique_categories2):
    subset = df2[df2['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[1], marker='o', s=s, label=label,alpha=alp)
# for i, category in enumerate(unique_categories3):
#     subset = df3[df3['Mask'] == category]
#     label = category.replace('.tif', '').replace("_", " ")
#     plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[2], marker='o', s=s, label=label,alpha=alp)
for i, category in enumerate(unique_categories4):
    subset = df4[df4['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[3], marker='o', s=s, label=label,alpha=alp)
for i, category in enumerate(unique_categories5):
    subset = df5[df5['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[4], marker='o', s=s, label=label,alpha=alp)
for i, category in enumerate(unique_categories6):
    subset = df6[df6['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[5], marker='o', s=s, label=label,alpha=alp)
for i, category in enumerate(unique_categories7):
    subset = df7[df7['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[6], marker='o', s=s, label=label,alpha=alp)
for i, category in enumerate(unique_categories8):
    subset = df8[df8['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[7], marker='o', s=s, label=label,alpha=alp)
for i, category in enumerate(unique_categories9):
    subset = df9[df9['Mask'] == category]
    label = category.replace('.tif', '').replace("_", " ")
    plt.scatter(subset['Point_X'], subset['Point_Y'], c=color[8], marker='o', s=s, label=label,alpha=alp)
